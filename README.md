# Searchless Chess: Mechanistic Interpretability Study ♟️🧠

This repository contains a mechanistic interpretability framework that reveals what chess concepts each layer of DeepMind's 270M-parameter Searchless Chess transformer has learned internally.
...
![Layer accuracy curves](src/interpretability/plots/layer_accuracy_curves.png)
...
![Concept heatmap](src/interpretability/plots/concept_heatmap.png)

### t-SNE: How the Model Organizes Position Space

![t-SNE plot](src/interpretability/plots/tsne_layer8.png)
...
![Per concept bars](src/interpretability/plots/per_concept_bars.png)
...
## 💻 Engine & Extension Backend

In addition to the interpretability study, this repository contains the backend for the **Chessbridge Chrome extension**, which runs the local 270M Transformer model to instantly compute chess moves using a WebSocket pipeline.

### How to Run the Chrome Extension Backend

1. **Start the Server**: The backend engine runs inside a Jupyter Notebook and hosts a WebSocket server on port 8000.
2. Open your Mac Terminal.
3. Activate your conda environment and start Jupyter Notebook:
   ```bash
   conda activate searchless
   cd /Users/shanesarosh/searchless_chess/src
   jupyter notebook
   ```
4. A new tab will open in Google Chrome showing the Jupyter directory. Click on **`searchless_chess.ipynb`**.
